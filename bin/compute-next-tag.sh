...
#
# Tags look like `v<Prometheus blackbox exporter version>-<release>`:
#
# - if defaults/main.yml points at a Prometheus blackbox exporter version that
#   has never been released, the release counter restarts at 0 (`v0.28.0-0`)
# - otherwise the counter is incremented (`v0.28.0-1`), but only if something
...
version="$(sed -nE 's|^prometheus_blackbox_exporter_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Prometheus blackbox exporter version from $defaults_path"
	exit 1
fi

# The version values carry a leading `v` (e.g. `v0.28.0`), and so do the tags
# (`v0.28.0-0`). Stripping any `v` before prepending one keeps this correct
# even if the version values ever stop carrying one.
